"""MoE routing analysis for Qwen3-30B-A3B.

Hooks into Qwen3-30B-A3B router (gate) at every layer, records top-8 expert
assignments for thinking tokens, then compares expert usage distributions
across (confident/uncertain x syco/nonsyco) groups.

Requires GPU (single forward pass per record, no generation).

Usage:
    CUDA_VISIBLE_DEVICES=0 python scripts/exp_moe_routing_qwen_moe.py
"""
...
from src.config import (
    GENERATED_DIR, QWEN_MOE_MODEL, QWEN_MOE_NUM_LAYERS, PLOTS_DIR, RESULTS_DIR,
)
from src.lib import (
    find_thinking_span,
    load_model,
...
NUM_EXPERTS = 128
TOP_K = 8
NUM_LAYERS = QWEN_MOE_NUM_LAYERS  # 48
...
def main():
    ...
    gen_dir = GENERATED_DIR / "sycophancy_qwen_moe"
    ...
    for layer_idx in range(NUM_LAYERS):
        ...
        h = gate.register_forward_hook(make_hook(layer_idx))
        hooks.append(h)
    ...
    with torch.no_grad():
        for q in tqdm(test_qids, desc="routing", unit="ex"):
            ...
            try:
                ...
                freqs = {}
                for layer_idx in range(NUM_LAYERS):
                    indices = router_data[layer_idx]  # (seq_len, top_k) or (1, seq_len, top_k)
                    if indices.ndim == 3:
                        indices = indices[0]
                    thinking_indices = indices[start_idx:end_idx, :]
    ...
    print("-" * 42)
    for l in range(NUM_LAYERS):
        print(f"L{l:<6} {js_conf_unc[l]:.6f}        {js_syco_nonsyco[l]:.6f}")

    # C. Jaccard overlap of top-16 dominant experts (scaled for 128 experts)
    top_n = 16
    jaccard_conf_unc = []
    ...
    ax.plot(layers, js_conf_unc, "o-", color="#1565C0", label="Confident vs Uncertain", linewidth=2, markersize=3)
    ax.plot(layers, js_syco_nonsyco, "s--", color="#C62828", label="Syco vs Non-syco", linewidth=2, markersize=3)
    ax.set_xlabel("Layer")
    ax.set_ylabel("Jensen-Shannon Divergence")
    ax.set_title("MoE Routing Divergence — Qwen3-30B-A3B")
    ax.legend()
    fig.tight_layout()
    div_path = PLOTS_DIR / "moe_routing_divergence_qwen_moe.png"
    fig.savefig(div_path, dpi=150)
    plt.close(fig)
    print(f"\nSaved {div_path}", flush=True)

    fig, ax = plt.subplots(1, 1, figsize=(12, 5))
    ax.plot(layers, jaccard_conf_unc, "o-", color="#1565C0", label="Confident vs Uncertain", linewidth=2, markersize=3)
    ax.plot(layers, jaccard_syco_nonsyco, "s--", color="#C62828", label="Syco vs Non-syco", linewidth=2, markersize=3)
    ax.set_xlabel("Layer")
    ax.set_ylabel(f"Jaccard Similarity (top-{top_n} experts)")
    ax.set_title("Expert Set Overlap — Qwen3-30B-A3B")
    ax.legend()
    ax.set_ylim(0, 1.05)
    fig.tight_layout()
    jac_path = PLOTS_DIR / "moe_routing_jaccard_qwen_moe.png"
    ...
    out_path = RESULTS_DIR / "exp_moe_routing_qwen_moe.json"
