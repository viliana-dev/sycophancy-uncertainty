"""Precompute mean-pooled features from Qwen3-30B-A3B activation files.

Same logic as precompute_features.py but with Qwen3-30B-A3B paths and hidden_size.

Output: data/features/sycophancy_qwen_moe/L{layer}_K{k}.npz
        keys: 'X' (n, 2048), 'qids' (n,)

Usage:
    python scripts/precompute_features_qwen_moe.py [--n-workers 16]
"""
...
def main():
    ...
    labeled_path = GENERATED_DIR / "sycophancy_qwen_moe" / "labeled.jsonl"
    ...
    out_dir = DATA_DIR / "features" / "sycophancy_qwen_moe"
    out_dir.mkdir(parents=True, exist_ok=True)

    matrices = {}
    for layer in layer_indices:
        for k in k_percents:
            matrices[(layer, k)] = np.zeros((n, QWEN_MOE_HIDDEN_SIZE), dtype=np.float32)
